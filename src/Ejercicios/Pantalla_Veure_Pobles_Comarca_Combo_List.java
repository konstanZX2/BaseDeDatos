...
public class Pantalla_Veure_Pobles_Comarca_Combo_List extends JFrame implements ActionListener, ListSelectionListener {
    ...
    private void agafarComarques() {
        ...
        try {
            ...
            ResultSet rs1 = st1.executeQuery("select nom_c from comarques c order by 1");
            while(rs1.next() ) {
    ...
    private void visualitzaCom(String comarca) {
        ...
        try {
            ...
            ResultSet rs2 = st2.executeQuery("select nom from poblacions where nom_c = '" + comarca.replaceAll("'","''") + "'");
            while(rs2.next() ) {
        ...
        String sql = "select nom from poblacions where nom_c = '" + comarca.replaceAll("'","''") + "'";
    ...
    private void visualitzaInstituts(String poble) {
        ...
        try {
            ...
            ResultSet rs3 = st3.executeQuery("select count(*) from poblacions inner join instituts using (cod_m) "
                    +"WHERE poblacions.nom = '" + poble.replaceAll("'","''")+"'");
            while(rs3.next() ) {
                peu.setText(rs3.getString(1) + "Instituts");
            }
            rs3.close();
            st3.close();
        } catch (SQLException e) {
